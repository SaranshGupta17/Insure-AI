...
async def get_claim_review_data(claim_id: str,user: dict = Depends(verify_employee_token)):
    try:
        ...
        response = supabase_safe.table("claims").select(
            "claim_id, customer_id, incident_type, claim_date_time, incident_date, incident_type, description, summary,  claim_status, rc_doc_url, fir_doc_url, ntr_doc_url, rto_doc_url, evidence_urls, customer(name,policy_no,vehicles(car_number,vehicle_model)) "
        ...
        print(f"Error fetching claim review data: {e}")
