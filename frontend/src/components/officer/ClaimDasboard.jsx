...
export default function ClaimDashboard({ claimId, onBack, setView }) {
  ...
  if (isLoading) {
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col items-center justify-center h-full min-h-[16rem] sm:min-h-125 px-4 text-center">
        ...
        <p className="text-slate-500 font-medium text-sm sm:text-base">Loading comprehensive claim dossier...</p>
      </div>
    );
  }
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col items-center justify-center h-full min-h-[16rem] sm:min-h-125 px-4 text-center">
        <div className="text-center text-slate-500">
          <p className="text-base sm:text-lg font-medium text-red-500 mb-2">{error || "Claim not found"}</p>
  ...
  const renderDocument = (urlData, title) => {
    ...
    return (
      <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex flex-col h-full">
        ...
        <div className="flex-1 w-full bg-slate-100 rounded-lg overflow-hidden flex items-center justify-center min-h-[12rem] sm:min-h-62.5 relative group">
          {isPdf ? (
            // 🔥 Fix 2: Use an <object> tag which handles PDFs better than <iframe> for signed URLs
            <object 
              data={url} 
              type="application/pdf" 
              className="w-full h-full min-h-[15rem] sm:min-h-75"
            >
            ...
            <img src={url} alt={title} className="w-full h-full object-cover max-h-[15rem] sm:max-h-75" loading="lazy" />
          )}
        </div>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        <button 
          onClick={onBack}
          className="flex items-center gap-2 text-slate-500 hover:text-slate-800 transition-colors font-medium shrink-0"
        >
        ...
        {claimData.claim_status === "Pending" ? (
          isRejecting ? (
            /* --- REJECTION INPUT UI --- */
            <div className="w-full sm:flex-1 sm:max-w-2xl bg-white p-3 rounded-xl border border-red-200 shadow-sm flex flex-col sm:flex-row items-stretch sm:items-start gap-3 animate-in fade-in slide-in-from-right-4 duration-1000">
              <textarea 
              ...
              />
              <div className="flex flex-row sm:flex-col gap-2 sm:min-w-28">
                <button 
                  disabled={!rejectionReason.trim()}
                  onClick={() => handleApproval(claimData.claim_id, "Rejected", rejectionReason)}
                  className="flex-1 sm:flex-none px-3 py-2 bg-red-600 text-white text-sm font-bold rounded-lg hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  Confirm Reject
                </button>
                <button 
                  ...
                  className="flex-1 sm:flex-none px-3 py-2 bg-slate-100 text-slate-600 text-sm font-bold rounded-lg hover:bg-slate-200 transition-colors"
                >
                  Cancel
                </button>
              </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 flex-1 overflow-y-auto overflow-hidden">
        ...
        {/* Header */}
        <div className="bg-slate-800 text-white p-4 sm:p-6 border-b border-slate-200 flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-end">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold font-mono tracking-tight break-all">{claimData.claim_id}</h2>
          ...
          
            {
              ...
              <div className='flex flex-col gap-1 items-start sm:h-full sm:justify-around sm:items-end'>
              ...
              <div className='flex flex-col gap-1 items-start sm:h-full sm:justify-between sm:items-end'>
        ...
        <div className="p-4 sm:p-6 grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
          ...
          <div className="space-y-6 sm:space-y-8">
            ...
            <section>
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-4 pb-2 border-b border-slate-200 flex items-center gap-2">
                <svg className="w-4 h-4 text-portal-teal shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                1. Customer Details
              </h3>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-3">
                <div className="flex flex-col gap-0.5 sm:flex-row sm:justify-between sm:gap-2">
                  ...
                  <span className="font-mono font-medium text-slate-800 break-all">{claimData.customer_id}</span>
                </div>
                <div className="flex flex-col gap-0.5 sm:flex-row sm:justify-between sm:gap-2">
                ...
                <div className="flex flex-col gap-0.5 sm:flex-row sm:justify-between sm:gap-2">
                  ...
                  <span className="font-mono font-medium text-portal-teal break-all">{claimData.policy_no}</span>
                </div>
              </div>
            </section>
            ...
            <section>
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-4 pb-2 border-b border-slate-200 flex items-center gap-2">
                <svg className="w-4 h-4 text-portal-teal shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>
                3. Vehicle Details
              </h3>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-3">
                <div className="flex flex-col gap-0.5 sm:flex-row sm:justify-between sm:gap-2">
                  <span className="text-slate-500 text-sm">Registration No.</span>
                  <span className="font-mono font-bold text-slate-800 bg-white px-2 py-0.5 rounded border self-start sm:self-auto">{claimData.car_number || 'N/A'}</span>
                </div>
                <div className="flex flex-col gap-0.5 sm:flex-row sm:justify-between sm:gap-2">
            ...
            <section>
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-4 pb-2 border-b border-slate-200 flex items-center gap-2">
                <svg className="w-4 h-4 text-portal-teal shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                5. Incident Details
              </h3>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-4">
                <div className="flex flex-col gap-0.5 sm:flex-row sm:justify-between border-b border-slate-200 pb-2">
          ...
          <div className="h-full">
            ...
            <section className="h-full flex flex-col">
              ...
              <div className="bg-indigo-50/50 p-4 sm:p-5 rounded-2xl border border-indigo-100 flex-1">
                
                {/* 2.1 Policy Match Badge */}
                <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between bg-white p-3 rounded-xl border border-indigo-100 shadow-sm">
                  <span className="text-sm font-semibold text-slate-600">Policy Coverage Status</span>
                  {claimData.summary?.is_covered === true ? (
                     <span className="self-start sm:self-auto px-3 py-1 bg-emerald-100 text-emerald-700 text-xs font-bold uppercase rounded-lg">Covered Match</span>
                  ) : claimData.summary?.is_covered === false ? (
                     <span className="self-start sm:self-auto px-3 py-1 bg-red-100 text-red-700 text-xs font-bold uppercase rounded-lg">Not Covered</span>
                  ) : (
                     <span className="self-start sm:self-auto px-3 py-1 bg-slate-200 text-slate-600 text-xs font-bold uppercase rounded-lg">Awaiting Analysis</span>
                  )}
                </div>
                    ...
                    {claimData.summary?.policy_references?.length > 0 ? (
                      claimData.summary.policy_references.map((ref, idx) => (
                        <li key={idx} className="flex gap-2 text-sm text-slate-600 bg-white p-3 rounded-xl border border-indigo-100 shadow-sm">
                          ...
                          <span className="font-mono text-xs leading-relaxed break-all">{ref}</span>
                        </li>
                      ))
                    ) : (
        ...
        <div className="p-4 sm:p-6 border-t border-slate-200 bg-slate-50">
          <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-6 pb-2 border-b border-slate-200 flex items-center gap-2">
            <svg className="w-5 h-5 text-portal-teal shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" /></svg>
          ...
          {!claimData.rc_url && !claimData.fir_url && !claimData.ntr_url && !claimData.rto_url && (!claimData.evidence_urls || claimData.evidence_urls.length === 0) && (
            <div className="text-center p-6 sm:p-8 text-slate-500 italic bg-white rounded-xl border border-slate-200">
